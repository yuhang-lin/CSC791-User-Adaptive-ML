...
def build_model(n_length, n_features, n_outputs, individual_training=False):
    ...
    if individual_training:
        ...
        model.add(TimeDistributed(MaxPooling1D(pool_size=2)))
        model.add(TimeDistributed(Flatten()))
        model.add(LSTM(100, activation='tanh', recurrent_regularizer=l2(0.05)))
...
if individual_training:
    exportCSV(accuracies_2, "accuracy_cnnLSTM_indiv.csv")
    exportCSV(f1_scores_2, "f1_cnnLSTM_indiv.csv")
else:
    exportCSV(accuracies_2, "accuracy_cnnLSTM_agg.csv")
    exportCSV(f1_scores_2, "f1_cnnLSTM_agg.csv")
